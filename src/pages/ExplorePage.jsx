...
const ExplorePage = () => {
    ...
    const [location, setLocation] = useState({ city: 'Salt Lake City', state: 'Utah' });
    ...
    const fetchEventsByCategories = async () => {
        ...
        for (const category of CATEGORIES) {
            try {
                const cachedData = localStorage.getItem(`events_${category.name}_${location.city}`);
                const cachedTimestamp = localStorage.getItem(`events_${category.name}_${location.city}_timestamp`);
                ...
                if (response.data._embedded && response.data._embedded.events) {
                    ...
                    localStorage.setItem(`events_${category.name}_${location.city}`, JSON.stringify(newEvents[category.name]));
                    localStorage.setItem(`events_${category.name}_${location.city}_timestamp`, new Date().getTime().toString());
                } else {
                            ...
                            <div className="absolute inset-0 flex flex-col-reverse md:flex-row h-full">
                                <div className="bg-gradient-to-r from-indigo-400 to-cyan-400 w-3/5 sm:w-[50%] h-full flex flex-col justify-center p-6 sm:p-10 clip-slant">
                                    ...
                                    <p className='text-white max-w-sm mb-6 [text-shadow:_0_2px_0_rgb(0_0_0_/25%)] text-sm sm:text-[1.05rem]'>
                                        Looking for something to do in {location.city}? Whether you're a local, new in town or just cruising through we've got loads of great tips and events.
                                    </p>
